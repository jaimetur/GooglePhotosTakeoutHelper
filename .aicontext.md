...
- **Source Preservation**: Original takeout files are never modified unless explicitly using `--copy` mode
- **Copy Mode Protection**: When `--copy` flag is used, original data is **NEVER** touched, modified, moved, or deleted under ANY circumstances. This is an absolute guarantee - the tool operates only on copies, ensuring complete preservation of original takeout data
- **Duplicate Removal Safety**: Duplicate detection uses SHA-256 content hashing for accuracy. In copy mode, duplicates are identified but originals remain untouched - only the copies in the output structure have duplicates removed. Without copy mode, duplicate removal moves (not deletes) files, so originals are relocated but not lost
...
**`--copy --albums duplicate-copy`**
- Maximum safety: originals preserved, albums have real copies
- Highest disk usage (3x space: originals + date structure + album copies)
- Recommended for irreplaceable collections

**`--copy --albums shortcut`**
- Originals preserved, albums use shortcuts to date-organized copies
- Medium disk usage (2x space: originals + organized copies)
- Good balance of safety and efficiency

**Without `--copy` + `--albums nothing`**
- Moves files from takeout to date structure only
- ⚠️ **RISK**: Original takeout structure is destroyed
- ⚠️ **DATA LOSS**: Archive/trash photos are lost

#### Duplicate Removal Behavior

**With `--copy` Flag** (Safest Option)
- Original takeout files remain completely untouched
- Duplicates identified using SHA-256 content hashing
- Only the copies in the output structure have duplicates removed
- If duplicate found: one copy kept in output, additional copies discarded
- Original duplicate files remain intact in takeout structure

**Without `--copy` Flag** (More Efficient, Less Safe)
...
- ⚠️ **RISK**: Original takeout organization is destroyed
...
1. **`--albums nothing`** (any other flags)
   - ⚠️ Archive/trash photos will be lost
   - Must warn user explicitly

2. **No `--copy` + destructive operations**
   - Original takeout structure will be modified
   - Should recommend `--copy` for irreplaceable photos
