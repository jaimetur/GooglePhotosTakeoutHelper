...
class DuplicateDetectionService with LoggerMixin {
      ...
      if (sameSize.value.length <= 1) {
        ...
        for (int i = 0; i < mediaWithSameSize.length; i += hashBatchSize) {
          ...
          if (mediaWithSameSize.length > 100) {
            ...
            (processed / mediaWithSameSize.length * 100).clamp(0, 100);
  ...
  Future<Map<String, List<MediaEntity>>> groupIdenticalFast(
    ...
    // logDebug('[Step 3/8] Starting FAST duplicate detection for ${mediaList.length} files...');
    ...
    // logDebug('[Step 3/8] FAST mode: ${sizeGroups.length} size groups');
    ...
    // logDebug('[Step 3/8] FAST duplicate detection completed in ${stopwatch.elapsed.inMilliseconds}ms');
    // logDebug('[Step 3/8] FAST mode: files hashed fully: $fullHashes (remainder filtered by fingerprint)');
    // logDebug('[Step 3/8] FAST mode: files with unique sizes: $uniqueBySize');
    // logDebug('[Step 3/8] Cache statistics: $cacheStats');
  ...
  /// - Groups by content using groupIdenticalFast (fallback to groupIdentical)
  ...
  /// - Emits telemetry and detailed logs
      ...
      for (final entry in extBuckets.entries) {
        ...
        for (int i = 0; i < qbLists.length; i += maxWorkersHash) {
          ...
          final batchResults = await Future.wait(slice.map((final q) async {
            ...
            try {
              hashGroups = await (duplicateService as dynamic).groupIdenticalFast(q);
            } catch (_) {
          ...
          for (final res in batchResults) {
            ...
            for (final entry in res.groups.entries) {
              ...
              final bool verifyThisGroup = verify && (group.length >= 4 || sizeKey > (64 << 20));
              ...
              if (verifyThisGroup) {
                try {
                  ...
                  final String keptHash =
                      expectedHash ?? await verifier.calculateFileHash(kept.primaryFile.asFile());
